...
# Load model
inp_file = 'C:/Users/dmi002/Desktop/Python WIP/thesis_project/SCOPEv1_results/Dili_main_results/Dili_Main.inp'
wn = wntr.network.WaterNetworkModel(inp_file)

print(len(wn.pipe_name_list))

orig_diameters = {pipe: wn.get_link(pipe).diameter * 1000 for pipe in wn.pipe_name_list}

df = pd.DataFrame.from_dict(orig_diameters, orient='index')
                  
sim = wntr.sim.EpanetSimulator(wn)
results = sim.run_sim()
junctions_demand_gt20 = results.node['pressure'].min()[results.node['pressure'].min().gt(20)].index.tolist()
...
def Min_Cost_GA(x):
    ...
    min_p = results.node['pressure'].loc[:, junctions_demand_gt20].min().min()
