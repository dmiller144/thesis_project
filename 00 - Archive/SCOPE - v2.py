...
# Load model
INP_file_name = 'Dili_LHS_main_noPRVsSCOPEv2' # input('Enter the filename of the network you wish to process: ')
...
junctions_w_demand = results.node['demand'][results.node['demand'].gt(0)].dropna(axis=1, how='all').columns.tolist()
junctions_0_demand = results.node['demand'][results.node['demand'].eq(0)].dropna(axis=1, how='any').columns.tolist()

min_p = results.node['pressure'].loc[:, junctions_w_demand].min().min()
min_p_0_demand = results.node['pressure'].loc[:, junctions_0_demand].min().min()
...
for j in junctions:
    ...
    min_p = results.node['pressure'].loc[:, junctions_w_demand].min().min()
    min_p_0_demand = results.node['pressure'].loc[:, junctions_0_demand].min().min()
    crit_p = min_p if min_p - min_p_req < min_p_0_demand else min_p_0_demand + min_p_req if min_p_0_demand > 0 else min_p_0_demand
    ...
    while abs(crit_p - min_p_req) > 0.1:
        while abs(crit_p - min_p_req) > 0.1:
            ...
            Duty_Head = Duty_Head + min_p_req - crit_p + 0.1
            wn.get_curve(curve).points = [(Duty_Flow, Duty_Head)]
