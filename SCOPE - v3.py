...
# Load model
INP_file_name = 'Dili_LHS_main_noPRVsSCOPEv3' # input('Enter the filename of the network you wish to process: ')
...
junctions_0_demand = results.node['demand'][results.node['demand'].eq(0)].dropna(axis=1, how='any').columns.tolist()
original_elevations = {name: wn.get_node(name).elevation for name in junctions_0_demand}
...
for j in junctions:
    #Lowering 0 demand junctions min_P_req
    for k in junctions_0_demand:
        wn.get_node(k).elevation = wn.get_node(k).elevation - min_p_req + tank_min_lvl
    ...
    while abs(min_p - min_p_req) > 0.1:
        ...
        # This loop applies the flow balancing method is the standard deviation of the pump flows is greater than 2
        while pump_flows.std() > 2:
            ...
            Duty_Head = results.node['pressure'].loc[:, pump_end_node].max()+5000
            wn.get_curve(curve).points = [(Duty_Flow, Duty_Head)]
    ...
    tank_init_lvl = wn.get_node('Balancing_tank').init_level
    for k in junctions_0_demand:
        wn.get_node(k).elevation = original_elevations[k]
